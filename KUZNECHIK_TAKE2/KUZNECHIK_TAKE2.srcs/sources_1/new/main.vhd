...
begin
   ...
   --workBlock <= inputBlock xor leftKey;
    --workBlock <= inputBlock;
    ...
    begin 
        ...
        for i in 0 to 16 loop
            eightBitBlock <= to_integer(unsigned(workBlock(8*i to (8*(i+1)-1))));
            wait for 100 ns;
            temp <= to_unsigned(constss(eightBitBlock), 8);
            wait for 100 ns;
            tempLogVec <= std_logic_vector(temp);
            wait for 100 ns;
            workBlock(8*i to ((8*(i+1))-1)) <= tempLogVec;
            wait for 100 ns;
        end loop;
    end process;
    
    --outputBlock <= workBlock;
end Behavioral;
